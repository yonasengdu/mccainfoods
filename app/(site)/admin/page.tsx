      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-8">
        ...
        <div className={`grid transition-all duration-500 ease-in-out ${formOpen ? "grid-rows-[1fr] opacity-100 mb-5 sm:mb-8" : "grid-rows-[0fr] opacity-0 mb-0"}`}>
          <div className="overflow-hidden">
            <div id="admin-applicant-form" className="bg-white rounded-2xl sm:rounded-3xl shadow-xl shadow-gray-200/50 border border-gray-100 p-4 sm:p-8 scroll-mt-24">
              <div className="flex items-center gap-3 mb-5 sm:mb-6">
                ...
                <div>
                  <h2 className="text-base sm:text-lg font-bold text-gray-900">{editingEmployeeId ? "Edit applicant" : "New Applicant"}</h2>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-x-5 sm:gap-y-4">
                    <div>
                      ...
                      <div className="flex gap-2">
                        ...
                        <input type="text" inputMode="text" value={phoneLocal} onChange={(e) => { setPhoneLocal(e.target.value); if (fieldErrors.phoneNumber) setFieldErrors((p) => ({ ...p, phoneNumber: undefined })); }} placeholder="Phone or contact" className={`flex-1 min-w-0 ${inputClass(!!fieldErrors.phoneNumber)}`} />
                      </div>
                  ...
                  <button type="submit" disabled={submitting} className="w-full sm:w-auto inline-flex items-center justify-center gap-2.5 bg-gradient-to-r from-mccain-green to-mccain-green-dark disabled:from-gray-300 disabled:to-gray-400 text-white font-semibold px-8 py-3.5 sm:py-3 rounded-2xl transition-all text-sm shadow-lg shadow-mccain-green/20 disabled:shadow-none active:scale-95">
                    {submitting ? (
                      <>
                        ...
                        {editingEmployeeId ? "Saving…" : "Adding…"}
                      </>
                    ) : editingEmployeeId ? (
                      "Save changes"
                    ) : (
                        ...
                        <tr key={emp.id} className={`transition-colors duration-150 ${cfg.row}`}>
                          ...
                          <td className="px-4 py-4 text-right">
                            <div className="flex items-center justify-end gap-2">
                              <button
                                type="button"
                                onClick={() => startEditApplicant(emp)}
                                className="inline-flex items-center gap-1 px-3 py-1.5 rounded-xl text-xs font-medium text-mccain-green hover:text-mccain-green-dark hover:bg-emerald-50 transition-all"
                              >
                              ...
                              <button type="button" onClick={() => handleDelete(emp.id)} className="inline-flex items-center gap-1 px-3 py-1.5 rounded-xl text-xs font-medium text-red-500 hover:text-red-700 hover:bg-red-50 transition-all">
                      ...
                      <div className="p-3.5 active:bg-gray-50 transition-colors">
                        ...
                        <div className="flex items-center gap-2">
                          <select
                            ...
                            className={`min-w-0 flex-1 text-xs font-semibold border rounded-xl px-3 py-2.5 focus:outline-none ${selectChevron} pr-8 ${cfg.badge}`}
                          >
                          ...
                          <button
                            type="button"
                            onClick={() => startEditApplicant(emp)}
                            className="flex shrink-0 items-center justify-center w-10 h-10 rounded-xl border border-emerald-200 text-mccain-green hover:bg-emerald-50 active:bg-emerald-100 transition-all"
                          ...
                          <button
                            ...
                            className="flex shrink-0 items-center justify-center w-10 h-10 rounded-xl border border-red-200 text-red-500 hover:bg-red-50 active:bg-red-100 transition-all"
